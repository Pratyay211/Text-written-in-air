...
class Model: 
	...
	def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False):
		"init model: add CNN, LSTM and CTC and initialize TF"
		...
		# LSTM
		lstmOut3d = self.setupLSTM(cnnOut4d)

		# CTC
		(self.loss, self.decoder) = self.setupCTC(lstmOut3d)
	...
	def setupLSTM(self, lstmIn4d):
		"create LSTM layers and return output of these layers"
		lstmIn3d = tf.squeeze(lstmIn4d, axis=[2])

		# basic cells which is used to build LSTM
		numHidden = 256
		cells = [tf.contrib.rnn.LSTMCell(num_units=numHidden, state_is_tuple=True) for _ in range(2)] # 2 layers

		# stack basic cells
		stacked = tf.contrib.rnn.MultiLSTMCell(cells, state_is_tuple=True)

		# bidirectional LSTM
		# BxTxF -> BxTx2H
		((fw, bw), _) = tf.nn.bidirectional_dynamic_rnn(cell_fw=stacked, cell_bw=stacked, inputs=lstmIn3d, dtype=lstmIn3d.dtype)
	...
	def setupCTC(self, ctcIn3d):
		...
		# return a CTC operation to compute the loss and a CTC operation to decode the LSTM output
